...
class pathCreator:
	def __init__(self, imagePath, numNails=200, maxLines=1500, stringOpacity=35, skipNeighbors=15, shape="circle"):
...
class loadedPath:
	def __init__(self,fName,totalLines=None, stringOpacity=None):
		...
		self.pc = pathCreator(o['imageName'],o['numNails'],lines,opacity,o['skipNeighbors'])
...
if __name__ == '__main__':
	imgPath = "IMG_7148.JPG"
	num = 3

	load = False
	loadPath = "IMG_7148.JPG-save-1"
	...
	doOutput = True
	if not load:
		pc = pathCreator(imgPath, numNails, maxLines, stringOpacity, skipNeighbors, shape)
		pc.generate(bgColor,stringColor, doOutput)
		pc.savePath(num)
	else:
		lp = loadedPath(loadPath+'-save-'+str(num),maxLines,stringOpacity)
		lp.drawImage(bgColor,stringColor, doOutput)
		lp.showPath()
